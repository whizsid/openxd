...
use super::{
    coordinates::{
        canvas_to_fb, canvas_to_ndc, screen_to_canvas, CanvasScope, FbScope, NdcScope, ScreenScope,
    },
    line::{Line, LineRenderPipeline},
    rectangle::{Rectangle, RectangleRenderPipeline},
...
pub struct Workbook {
    ...
    transform_to_ndc: Transform2D<f64, CanvasScope, NdcScope>,
    /// Canvas to framebuffer transformer
    transform_to_fb: Transform2D<f64, CanvasScope, FbScope>,
    /// To render lines
    line_render_pipeline: LineRenderPipeline,
    rectangle_render_pipeline: RectangleRenderPipeline,
...
impl Workbook {
    pub fn new(render_state: &RenderState, ppcm: f32) -> Workbook {
        ...
        let rectangle_render_pipeline = RectangleRenderPipeline::new(&render_state, vec![]);
    ...
    pub fn reset(&mut self, screens: Vec<ScreenWithChild>) {
        ...
        self.rectangle_render_pipeline.reset();
            ...
            match screen.items {
                ScreenItems::Items(items) => {
                    ...
                    for item in items {
                        match item {
                            Item::Line(line) => {
                                let line_index = self.line_render_pipeline.add(line.to_line_raw(
                                    screen.meta.clone(),
                                    sndc_transform,
                                    sfb_transform,
                                ));
                            ...
                            Item::Rectangle(rect) => {
                                let rect_index =
                                    self.rectangle_render_pipeline.add(rect.to_rectangle_raw(
                                        screen.meta.clone(),
                                        sndc_transform,
                                        sfb_transform,
                                    ));
                                indexed_items.push(IndexedItem::Rectangle {
                                    rectangle: rect,
                                    rectangle_index: rect_index,
                                });
                            }
                        }
                    }
    ...
    fn get_sndc_sfb_transformations(
        &self,
        screen: &Screen,
    ) -> (
        Transform2D<f64, ScreenScope, NdcScope>,
        Transform2D<f64, ScreenScope, FbScope>,
    ) {
        let min = screen.tl();
        let sc_transform = screen_to_canvas(screen.get_ppcm(), min.x, min.y);
        let sndc_transform = sc_transform.then(&self.transform_to_ndc);
        let sfb_transform = sc_transform.then(&self.transform_to_fb);
        (sndc_transform, sfb_transform)
    }

    pub fn add_line(&mut self, screen: Screen, line: Line) {
        let (sndc_transform, sfb_transform) = self.get_sndc_sfb_transformations(&screen);
        let line_raw = line.to_line_raw(screen, sndc_transform, sfb_transform);
        self.line_render_pipeline.add(line_raw);
    }

    pub fn add_rectangle(&mut self, screen: Screen, rectangle: Rectangle) {
        let (sndc_transform, sfb_transform) = self.get_sndc_sfb_transformations(&screen);
        let rect_raw = rectangle.to_rectangle_raw(screen, sndc_transform, sfb_transform);
        self.rectangle_render_pipeline.add(rect_raw);
    }
    ...
    pub fn scroll(&mut self, offset_x: f64, offset_y: f64) {
    ...
    pub fn prepare(&mut self, device: &Device, queue: &Queue) {
        ...
        self.rectangle_render_pipeline.prepare(device, queue);
    }

    pub fn paint<'rpass>(&'rpass self, rpass: &mut RenderPass<'rpass>) {
        self.line_render_pipeline.paint(rpass);
        self.rectangle_render_pipeline.paint(rpass);
    }
}
